...
//			ResultSet result = st.executeQuery("select * from tottenham_squad where p_number=10");	//전송 할 sql 문
			ResultSet result = st.executeQuery("select * from tottenham_squad where p_position='공격수'");	//전송 할 sql 문
			while (result.next()) {	// 결과를 하나씩 빼기. 더 이상 없으면(행 수가 끝나면) false 리턴됨.
				...
				String number = result.getString("p_number");	// p_number 필드(열) 의 데이터 꺼내기(1개 꺼낸거에서)
				System.out.println(number);
//				String w_pay = result.getString("weekly_pay");	// weekly_pay 필드 의 데이터 꺼내기 (전부 값을 넣어준게 아니어서 null이 나오는애도 있음)
//				System.out.println(w_pay);
			}
